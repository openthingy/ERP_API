...
sudo apt install wget gnupg2 ca-certificates lsb-release ubuntu-keyring software-properties-common -y > /dev/null
if [ $? -ne 0 ]; then
...
echo deb [arch=amd64,arm64 signed-by=/usr/share/keyrings/nginx-archive-keyring.gpg] http://nginx.org/packages/mainline/ubuntu $codename nginx | sudo tee /etc/apt/sources.list.d/nginx-mainline.list
...
echo -e "Package: *\nPin: origin deb.nodesource.com\nPin-Priority: 900\n" | sudo tee /etc/apt/preferences.d/99nodesource > /dev/null
...
echo deb [arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-keyring.gpg] https://repo.mongodb.org/apt/ubuntu $codename/mongodb-org/5.0 multiverse | sudo tee /etc/apt/sources.list.d/mongodb-server.list
